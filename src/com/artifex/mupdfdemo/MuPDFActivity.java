...
public class MuPDFActivity extends Activity implements FilePicker.FilePickerSupport {
	...
	public void createAlertWaiter() {
		...
		mAlertTask = new AsyncTask<Void, Void, MuPDFAlert>() {
			...
			protected void onPostExecute(final MuPDFAlert result) {
				...
				case OkCancel:
					mAlertDialog.setButton(AlertDialog.BUTTON2, EUExUtil.getString("cancel"), listener);
					pressed[1] = MuPDFAlert.ButtonPressed.Cancel;
				case Ok:
					mAlertDialog.setButton(AlertDialog.BUTTON1, EUExUtil.getString("confirm"), listener);
	...
	public void onCreate(final Bundle savedInstanceState) {
		...
		gAlertBuilder = mAlertBuilder; // keep a static copy of this that other
										// classes can use
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		case PRINT_REQUEST:
			if (resultCode == RESULT_CANCELED)
				showInfo(EUExUtil.getString("plugin_uexpdfreader_print_failed"));
			break;
	...
	private void printDoc() {
		if (!core.fileFormat().startsWith("PDF")) {
			showInfo(EUExUtil.getString("plugin_uexpdfreader_format_currently_not_supported"));
			return;
		}
	...
	private void makeButtonsView() {
		...
		mInfoView = (TextView) mButtonsView.findViewById(EUExUtil.getResIdID("info"));
		mSearchButton = (ImageButton) mButtonsView.findViewById(EUExUtil.getResIdID("searchButton"));
		mReflowButton = (ImageButton) mButtonsView.findViewById(EUExUtil.getResIdID("reflowButton"));
		mOutlineButton = (ImageButton) mButtonsView.findViewById(EUExUtil.getResIdID("outlineButton"));
		mAnnotButton = (ImageButton) mButtonsView.findViewById(EUExUtil.getResIdID("editAnnotButton"));
		mAnnotTypeText = (TextView) mButtonsView.findViewById(EUExUtil.getResIdID("annotType"));
		mTopBarSwitcher = (ViewAnimator) mButtonsView.findViewById(EUExUtil.getResIdID("switcher"));
		mSearchBack = (ImageButton) mButtonsView.findViewById(EUExUtil.getResIdID("searchBack"));
		mSearchFwd = (ImageButton) mButtonsView.findViewById(EUExUtil.getResIdID("searchForward"));
		mSearchText = (EditText) mButtonsView.findViewById(EUExUtil.getResIdID("searchText"));
		mLinkButton = (ImageButton) mButtonsView.findViewById(EUExUtil.getResIdID("linkButton"));
		mMoreButton = (ImageButton) mButtonsView.findViewById(EUExUtil.getResIdID("moreButton"));
		mProofButton = (ImageButton) mButtonsView.findViewById(EUExUtil.getResIdID("proofButton"));
		mSepsButton = (ImageButton) mButtonsView.findViewById(EUExUtil.getResIdID("sepsButton"));
		mTopBarSwitcher.setVisibility(View.INVISIBLE);
		mPageNumberView.setVisibility(View.INVISIBLE);
		mInfoView.setVisibility(View.INVISIBLE);
	...
	public void OnSepsButtonClick(final View v) {
		if (isProofing()) {
			...
			// This makes the popup menu display icons, which by default it does
			// not do.
			// I worry that this relies on the internals of PopupMenu, which
			// could change.
			try {
	...
	public void OnCopyTextButtonClick(View v) {
		...
		mAnnotTypeText.setText(EUExUtil.getString("plugin_uexpdfreader_copy_text"));
		showInfo(EUExUtil.getString("plugin_uexpdfreader_select_text"));
	}
	...
	public void OnHighlightButtonClick(View v) {
		...
		mDocView.setMode(MuPDFReaderView.Mode.Selecting);
		mAnnotTypeText.setText(EUExUtil.getString("plugin_uexpdfreader_highlight"));
		showInfo(EUExUtil.getString("plugin_uexpdfreader_select_text"));
	}
	...
	public void OnUnderlineButtonClick(View v) {
		...
		mDocView.setMode(MuPDFReaderView.Mode.Selecting);
		mAnnotTypeText.setText(EUExUtil.getString("plugin_uexpdfreader_underline"));
		showInfo(EUExUtil.getString("plugin_uexpdfreader_select_text"));
	}
	...
	public void OnStrikeOutButtonClick(View v) {
		...
		mDocView.setMode(MuPDFReaderView.Mode.Selecting);
		mAnnotTypeText.setText(EUExUtil.getString("plugin_uexpdfreader_strike_out"));
		showInfo(EUExUtil.getString("plugin_uexpdfreader_select_text"));
	}
	...
	public void OnInkButtonClick(View v) {
		...
		mAnnotTypeText.setText(EUExUtil.getString("plugin_uexpdfreader_ink"));
		showInfo(EUExUtil.getString("plugin_uexpdfreader_draw_annotation"));
	}
	...
	public void OnAcceptButtonClick(View v) {
		...
		case Highlight:
			...
			mTopBarMode = TopBarMode.Annot;
			if (!success)
				showInfo(EUExUtil.getString("plugin_uexpdfreader_no_text_selected"));
			break;
		...
		case Ink:
			...
			mTopBarMode = TopBarMode.Annot;
			if (!success)
				showInfo(EUExUtil.getString("plugin_uexpdfreader_nothing_to_save"));
			break;
		}
		mTopBarSwitcher.setDisplayedChild(mTopBarMode.ordinal());
		mDocView.setMode(MuPDFReaderView.Mode.Viewing);
	...
	public void onBackPressed() {
		if (core != null && core.hasChanges()) {
			...
			alert.setMessage(EUExUtil.getString("plugin_uexpdfreader_document_has_changes_save_them_"));
			alert.setButton(AlertDialog.BUTTON_POSITIVE, EUExUtil.getString("plugin_uexpdfreader_yes"), listener);
